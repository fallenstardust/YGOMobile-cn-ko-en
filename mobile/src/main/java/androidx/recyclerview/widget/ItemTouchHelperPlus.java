...
public class ItemTouchHelperPlus extends RecyclerView.ItemDecoration
     ...
     */
    @NonNull
    ItemTouchHelperPlus.Callback mCallback;
     ...
     * {@link ItemTouchHelperPlus.Callback#getAbsoluteMovementFlags(RecyclerView, RecyclerView.ViewHolder)} for the current
     * action state.
     */
    @SuppressWarnings("WeakerAccess") /* synthetic access */
            int mSelectedFlags;
     ...
     * jumpy UIs.
     */
    List<ItemTouchHelperPlus.RecoverAnimation> mRecoverAnimations = new ArrayList<>();
     ...
     */
    private ItemTouchHelperPlus.ItemTouchHelperGestureListener mItemTouchHelperGestureListener;
    ...
    private final OnItemTouchListener mOnItemTouchListener = new OnItemTouchListener() {
        ...
        public boolean onInterceptTouchEvent(@NonNull RecyclerView recyclerView,
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                if (mSelected == null) {
                    final ItemTouchHelperPlus.RecoverAnimation animation = findAnimation(event);
                    if (animation != null) {
     ...
     */
    public ItemTouchHelperPlus(@NonNull ItemTouchHelperPlus.Callback callback) {
        mCallback = callback;
    }
    ...
    private void destroyCallbacks() {
        ...
        for (int i = recoverAnimSize - 1; i >= 0; i--) {
            final ItemTouchHelperPlus.RecoverAnimation recoverAnimation = mRecoverAnimations.get(0);
    ...
    private void startGestureDetection() {
        mItemTouchHelperGestureListener = new ItemTouchHelperPlus.ItemTouchHelperGestureListener();
        mGestureDetector = new GestureDetectorCompat(mRecyclerView.getContext(),
    ...
    void select(@Nullable ViewHolder selected, int actionState) {
        ...
        if (mSelected != null) {
            ...
            if (prevSelected.itemView.getParent() != null) {
                ...
                final ItemTouchHelperPlus.RecoverAnimation rv = new ItemTouchHelperPlus.RecoverAnimation(prevSelected, animationType,
    ...
    @SuppressWarnings("WeakerAccess") /* synthetic access */
    void postDispatchSwipe(final ItemTouchHelperPlus.RecoverAnimation anim, final int swipeDir) {
...
//        }
    ...
    void endRecoverAnimation(ViewHolder viewHolder, boolean override) {
        final int recoverAnimSize = mRecoverAnimations.size();
        for (int i = recoverAnimSize - 1; i >= 0; i--) {
            final ItemTouchHelperPlus.RecoverAnimation anim = mRecoverAnimations.get(i);
     ...
     * {@link ItemTouchHelperPlus.Callback#isLongPressDragEnabled()}.
     * <p>
     ...
     * ItemTouchHelper
     * is attached.</li>
     * <li>{@link ItemTouchHelperPlus.Callback} must have dragging enabled.</li>
     ...
     *                 mItemTouchHelperPlus.startDrag(viewHolder);
     *             }
     *             return false;
     *         }
     *     });
     ...
     *                   RecyclerView.
     * @see ItemTouchHelperPlus.Callback#isItemViewSwipeEnabled()
     */
    public void startDrag(@NonNull ViewHolder viewHolder) {
        ...
        if (viewHolder.itemView.getParent() != mRecyclerView) {
            Log.e(TAG, "Start drag has been called with a view holder which is not a child of "
                    + "the RecyclerView which is controlled by this ItemTouchHelperPlus.");
            return;
        }
        obtainVelocityTracker();
        mDx = mDy = 0f;
     ...
     * behavior
     * by overriding {@link ItemTouchHelperPlus.Callback}
     * <p>
     * For this method to work:
     * <ul>
     * <li>The provided ViewHolder must be a child of the RecyclerView to which this
     * ItemTouchHelper is attached.</li>
     * <li>{@link ItemTouchHelperPlus.Callback} must have swiping enabled.</li>
     * <li>There must be a previous touch event that was reported to the ItemTouchHelper
     * through RecyclerView's ItemTouchListener mechanism. As long as no other ItemTouchListener
     * grabs previous events, this should work as expected.</li>
     * </ul>
     ...
     *         public boolean onTouch(View v, MotionEvent event) {
     *             if (MotionEvent.getActionMasked(event) == MotionEvent.ACTION_DOWN) {
     *                 mItemTouchHelperPlus.startSwipe(viewHolder);
     *             }
     *             return false;
     *         }
     *     });
    ...
    public void startSwipe(@NonNull ViewHolder viewHolder) {
        ...
        if (viewHolder.itemView.getParent() != mRecyclerView) {
            Log.e(TAG, "Start swipe has been called with a view holder which is not a child of "
                    + "the RecyclerView controlled by this ItemTouchHelperPlus.");
            return;
        }
        obtainVelocityTracker();
        mDx = mDy = 0f;
    ...
    @SuppressWarnings("WeakerAccess") /* synthetic access */
    ItemTouchHelperPlus.RecoverAnimation findAnimation(MotionEvent event) {
        ...
        if (Math.abs(mDx) > Math.abs(mDy)) {
            if ((swipeDir = checkHorizontalSwipe(viewHolder, flags)) > 0) {
                ...
                if ((originalFlags & swipeDir) == 0) {
                    // convert to relative
                    return ItemTouchHelperPlus.Callback.convertToRelativeDirection(swipeDir,
                            ViewCompat.getLayoutDirection(mRecyclerView));
                }
                return swipeDir;
            }
     ...
     * {@link ItemTouchHelperPlus.SimpleCallback}.
     * <p>
     * If user drags an item, ItemTouchHelper will call
     * {@link ItemTouchHelperPlus.Callback#onMove(RecyclerView, RecyclerView.ViewHolder, RecyclerView.ViewHolder)
    ...
    public abstract static class Callback {
        ...
        /**
         * Returns the {@link ItemTouchUIUtil} that is used by the {@link ItemTouchHelperPlus.Callback} class for
         * visual
         ...
         * <p>
         * By default, {@link ItemTouchHelperPlus.Callback} applies these changes on
         ...
         * </pre>
         *
         * @return The {@link ItemTouchUIUtil} instance that is used by the {@link ItemTouchHelperPlus.Callback}
         */
        @SuppressWarnings("WeakerAccess")
        @NonNull
        ...
        public void onMoved(@NonNull final RecyclerView recyclerView,
            ...
            if (layoutManager instanceof ItemTouchHelperPlus.ViewDropHandler) {
                ((ItemTouchHelperPlus.ViewDropHandler) layoutManager).prepareForDrop(viewHolder.itemView,
                        target.itemView, x, y);
                return;
            }
        ...
        void onDraw(Canvas c, RecyclerView parent, ViewHolder selected,
                    List<ItemTouchHelperPlus.RecoverAnimation> recoverAnimationList,
                    int actionState, float dX, float dY) {
            final int recoverAnimSize = recoverAnimationList.size();
            for (int i = 0; i < recoverAnimSize; i++) {
                final ItemTouchHelperPlus.RecoverAnimation anim = recoverAnimationList.get(i);
                anim.update();
         ...
         * is
         * achieved via {@link android.view.ViewGroup#(int, int)} and on L
     ...
     *     new ItemTouchHelperPlus.SimpleCallback(ItemTouchHelperPlus.UP | ItemTouchHelperPlus.DOWN,
     *         ItemTouchHelperPlus.LEFT) {
     ...
     * </pre>
     */
    public abstract static class SimpleCallback extends ItemTouchHelperPlus.Callback {
...
//         * {@link ItemTouchHelperPlus.ItemTouchHelperGestureListener#onLongPress(MotionEvent)}.
//         *
...
//         * {@link ItemTouchHelperPlus.ItemTouchHelperGestureListener#onLongPress(MotionEvent)} being called.
//         */
...
//    }
    ...
    public ItemTouchHelperPlus(Context context, ItemTouchHelperPlus.Callback2 callback) {
        this(callback);
        mContext = context;
        mCallback2 = callback;
    ...
    public abstract static class Callback2 extends ItemTouchHelperPlus.Callback {
        ...
        public void setItemTouchHelper(ItemTouchHelperPlus itemTouchHelper) {
            ...
            mHandler = new Handler(mItemTouchHelper.getContext().getMainLooper());
        }
        ...
        public void onSelectedChanged(RecyclerView.ViewHolder viewHolder, int actionState) {
            ...
            if (viewHolder != null && actionState == ItemTouchHelperPlus.ACTION_STATE_DRAG) {
